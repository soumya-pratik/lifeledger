...
import { PrimaryButton, Surface } from "@/shared/ui/chrome";
    ...
    <div className="space-y-6">
      <form onSubmit={(e) => void onSubmit(e)}>
        ...
        <div>
          ...
          <p className="mt-1 text-xs leading-relaxed text-ll-muted">
            CSV first (headers like Date, Debit, Narration). Credits are ignored so deposits do not
            become expenses. Rows land as proposals on the Ledger tab → From bank statements. Ledger:{" "}
            {ledger.name}.
          </p>
        </div>
        <input
          ...
          className="block w-full text-sm text-ll-muted file:mr-3 file:rounded-full file:border-0 file:bg-ll-bg file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-ll-text"
        />
        ...
        <PrimaryButton type="submit" disabled={!file || busy} className="w-full">
      ...
      <section>
        <h2 className="mb-3 text-sm font-semibold">Recent batches</h2>
        {batches.length === 0 ? (
          ...
          <ul className="divide-y divide-ll-border overflow-hidden rounded-2xl border border-ll-border bg-ll-surface shadow-[var(--ll-shadow)]">
            {batches.map((b) => (
              <li key={b.id} className="px-4 py-3 text-sm hover:bg-ll-bg">
